/**
 * Range Bot Engine - Production Ready (10/10)
 * Trading bot for sideways/ranging markets
 * 
 * Features:
 * - ADX-based auto range detection (determines trending vs ranging)
...
export interface RangeState {
  ...
  isTrending: boolean; // ADX-based trending detection
}
...
export interface RangeConfig {
  ...
  // ADX-based range detection (NEW - 10/10 feature)
  useADX: boolean;              // Use ADX to confirm ranging market
  adxPeriod: number;            // ADX calculation period
  adxTrendingThreshold: number; // ADX above this = trending (skip range trading)
  adxRangingThreshold: number;  // ADX below this = ranging (enable range trading)
...
export class RangeBot {
  ...
  /**
   * Calculate ADX (Average Directional Index) - 10/10 feature
