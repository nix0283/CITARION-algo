...
export class DCABotEngine extends EventEmitter {
  ...
  private handlePriceUpdate(price: number): void {
    ...
    // Store price history for Sharpe/Sortino calculation
    this.priceHistory.push(price);
    // Keep last 1000 prices (about 16 minutes at 1s interval)
  ...
  private calculateSharpeRatio(): number {
    ...
    // Rp = portfolio return, Rf = risk-free rate (0 for simplicity), σp = std dev of returns
    ...
    // Annualized Sharpe (assuming 1-minute intervals, 525600 minutes/year)
    const annualizationFactor = Math.sqrt(525600);
  ...
  private calculateSortinoRatio(): number {
    ...
    // Annualized Sortino
    const annualizationFactor = Math.sqrt(525600);
    return (meanReturn * annualizationFactor) / downsideDev;
  }
