 ...
 * 
 * Features:
 * - Real orderbook WebSocket stream (Binance)
...
class OrderbookWebSocket {
  ...
  private onOrderbook: (bids: [number, number][], asks: [number, number][]) => void
  ...
  constructor(
    config: HFTConfig,
    onOrderbook: (bids: [number, number][], asks: [number, number][]) => void
  ) {
    this.config = config
  ...
  connect(): void {
    ...
    console.log(`[HFT] Connecting to orderbook stream: ${wsUrl}`)

    try {
      this.ws = new WebSocket(wsUrl)

      this.ws.onopen = () => {
        console.log('[HFT] Orderbook WebSocket connected')
      ...
      this.ws.onclose = () => {
        console.log('[HFT] WebSocket closed')
        this.ws = null
        this.isConnecting = false
        this.reconnect()
      }
    } catch (error) {
      console.error('[HFT] Failed to connect:', error)
      this.isConnecting = false
      this.reconnect()
    }
  }
  ...
  private reconnect(): void {
    if (this.reconnectAttempts >= this.maxReconnectAttempts) {
      console.error('[HFT] Max reconnect attempts reached')
      return
    }

    this.reconnectAttempts++
    const delay = Math.min(1000 * Math.pow(2, this.reconnectAttempts), 30000)
    console.log(`[HFT] Reconnecting in ${delay}ms (attempt ${this.reconnectAttempts})`)
...
export class HFTEngine {
  ...
  private latestOrderbook: { bids: [number, number][]; asks: [number, number][] } | null = null
  private useRealOrderbook = true
  ...
  constructor(config: Partial<HFTConfig> = {}) {
    ...
    // Initialize real orderbook WebSocket
    this.orderbookWs = new OrderbookWebSocket(
      fullConfig,
      (bids, asks) => {
        this.latestOrderbook = { bids, asks }
      }
    )
  }
  ...
  async start(): Promise<void> {
    ...
    // Connect to real orderbook stream
    if (this.useRealOrderbook && this.orderbookWs) {
    ...
    console.log('[HFT] Engine started with real orderbook stream')
  ...
  async stop(): Promise<void> {
    ...
    // Disconnect orderbook WebSocket
    if (this.orderbookWs) {
  ...
  private async analysisCycle(): Promise<void> {
    ...
    try {
      // Use real orderbook if available, otherwise simulate
      let orderbook: { bids: [number, number][]; asks: [number, number][] }
      
      if (this.useRealOrderbook && this.latestOrderbook) {
        orderbook = this.latestOrderbook
      } else {
        orderbook = this.simulateOrderbook()
      }
